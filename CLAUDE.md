...
8. **Backend sincroniza via GitHub ↔ Lovable Cloud.** O Lovable faz pull do GitHub automaticamente — então **migrations adicionadas em `supabase/migrations/`** com timestamp UTC futuro **são detectadas e aplicadas** pelo Lovable na próxima sincronização. Pode editar SQL diretamente neste repo desde que: (a) timestamp único e cronológico no nome do arquivo (`YYYYMMDDHHMMSS_descricao.sql`); (b) SQL idempotente (`CREATE TABLE IF NOT EXISTS`, `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`, `DROP POLICY IF EXISTS` antes de criar); (c) `SET search_path = public` em toda função `SECURITY DEFINER`; (d) RLS sempre habilitado em tabelas novas. **Edge Functions** (`supabase/functions/`) também sincronizam — mesmas regras: nome de pasta único, `withErrorBoundary` no entry-point, secrets nunca commitados. Mudanças que precisem de configuração no painel do Lovable (secrets, OAuth scopes) ainda vão pro `backlog.md` com tag `[lovable]`.
